    ...
    <Layout>
      ...
      >
        ...
        <motion.h1
          ...
          variants={FADE_DOWN_ANIMATION_VARIANTS}
        >
          <Balancer>How much would you have made?</Balancer>
        </motion.h1>
        <motion.p
      ...
      <div className="my-10 w-full max-w-screen-xl animate-[slide-down-fade_0.5s_ease-in-out] grid-cols-1 gap-5 px-5 md:grid-cols-3 xl:px-0">
        {features.map(({ title, description, large }) => (
          <Card
            ...
            demo={
                              <motion.h1
          className="text-black flex font-display font-bold tracking-[-0.02em] text-transparent drop-shadow-sm md:text-7xl md:leading-[5rem]"
          variants={FADE_DOWN_ANIMATION_VARIANTS}
        >
          <Balancer>{calculateProfit(100,ethPriceData,1).toFixed(2)}%</Balancer>
        </motion.h1>
            }
            large={large}
          />
